...
def create_planar_subdivision():
    """
    Create a planar subdivision interactively with use of mouse.

    :return
    1. A list of all points
    2. A list of tuples of indices representing edges and diagonals between points
    """

    POINTS_MAKRERS_COLOR = 'b'
    POLYGON_SEGMENTS_COLOR = POINTS_MAKRERS_COLOR
    DIAGONAL_COLOR = 'g'

    def onclick(event):
        nonlocal POINTS_MAKRERS_COLOR, POLYGON_SEGMENTS_COLOR
        if event.button == 1:  # Left-click to add a point
            if current_stage[0] == 1:
                ...
                ax.plot(event.xdata, event.ydata, color = POINTS_MAKRERS_COLOR, marker = 'o')  # Red points for polygon

                # draw edge
                if len(all_points) > 1:
                    ax.plot([all_points[-2][0], all_points[-1][0]],
                            [all_points[-2][1], all_points[-1][1]], color = POLYGON_SEGMENTS_COLOR)

                plt.draw()
            elif current_stage[0] == 2:
                # Add inner points
                all_points.append((event.xdata, event.ydata))
                ax.plot(event.xdata, event.ydata, color = POINTS_MAKRERS_COLOR, marker = 'o')  # Blue points for inner points
                plt.draw()
                ...
                for i, point in enumerate(all_points):
                    if abs(event.xdata - point[0]) < 0.5 and abs(event.ydata - point[1]) < 0.5:
                        if diagonal_start_index[0] is None:
                            ...
                            ax.plot(point[0], point[1], color = DIAGONAL_COLOR, marker = 'o')  # Mark as selected
                            plt.draw()
                        else:
                            ...
                            if start != end and (start, end) not in diagonals and (end, start) not in diagonals:
                                ...
                                ax.plot([all_points[start][0], all_points[end][0]],
                                        [all_points[start][1], all_points[end][1]], color = DIAGONAL_COLOR, linestyle = '--')

                                diagonal_start_index[0] = None

                                ax.plot(all_points[start][0], all_points[start][1], color = POINTS_MAKRERS_COLOR, marker = 'o')
                                plt.draw()
                        break
            ...
            if current_stage[0] == 1:
                ...
                if len(all_points) > 2:
                    ...
                    ax.plot([all_points[-1][0], all_points[0][0]],
                            [all_points[-1][1], all_points[0][1]], color = POLYGON_SEGMENTS_COLOR, linestyle = '-')
                    plt.draw()
    ...
    fig, ax = plt.subplots()
    ax.set_title("Add bounding polygon\nRight-click to finish")
    ax.set_xlim(-10, 10)
    ax.set_ylim(-10, 10)
